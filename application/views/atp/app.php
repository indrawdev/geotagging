...
<div id="body">
	<div class="container">
    <h1>Management ATP (Approval) <span><form method="post" name="search_box" id="search-box" action="<?php echo base_url() ?>index.php/process/search/atp_app" onsubmit="return searchbox()"><input type="text" name="q" class="searchbox" maxlength="100" placeholder="Search.." value="<?php echo $search[0]->keyword ?>"></form><a href="<?php echo base_url() ?>index.php/main/atp_pending">&empty; Pending Approval</a> <a class="submit" href="<?php echo base_url() ?>index.php/main/atp">ATP Task</a></span></h1>
    ...
    <tr>
        ...
        <th align="left"><a href="<?php echo base_url() ?>index.php/main/atp_app/<?php echo $t ?>/<?php echo $p ?>/<?php echo $s ?>/<?php if($sort == 'title-desc') echo 'title-asc'; else echo 'title-desc' ?>">Task Title</a><?php if($sorted[0] == 'title'){ echo '<span>'; if($sorted[1] == 'asc') echo ' &and;'; else echo ' &or;'; echo '</span>'; } ?></th>
        <th width="200"><a href="<?php echo base_url() ?>index.php/main/atp_app/<?php echo $t ?>/<?php echo $p ?>/<?php echo $s ?>/<?php if($sort == 'site-desc') echo 'site-asc'; else echo 'site-desc' ?>">Site Name</a><?php if($sorted[0] == 'site'){ echo '<span>'; if($sorted[1] == 'asc') echo ' &and;'; else echo ' &or;'; echo '</span>'; } ?></th>
        <th width="150"><a href="<?php echo base_url() ?>index.php/main/atp_app/<?php echo $t ?>/<?php echo $p ?>/<?php echo $s ?>/<?php if($sort == 'doc-desc') echo 'doc-asc'; else echo 'doc-desc' ?>">Doc</a><?php if($sorted[0] == 'doc'){ echo '<span>'; if($sorted[1] == 'asc') echo ' &and;'; else echo ' &or;'; echo '</span>'; } ?></th>
        <th width="70"><a href="<?php echo base_url() ?>index.php/main/atp_app/<?php echo $t ?>/<?php echo $p ?>/<?php echo $s ?>/<?php if($sort == 'created-desc') echo 'created-asc'; else echo 'created-desc' ?>">Created</a><?php if($sorted[0] == 'created'){ echo '<span>'; if($sorted[1] == 'asc') echo ' &and;'; else echo ' &or;'; echo '</span>'; } ?></th>
    </tr>
    </thead>
    <tbody>
        ...
        <tr id="list_atp_<?php echo $list->atp_id ?>">
            ...
            <a rel="if_url" url="<?php echo base_url() ?>index.php/form/action/atp_task/<?php echo $list->atp_id ?>"><img src="<?php echo base_url() ?>static/i/icon_tick.png" title="Approve" style="vertical-align:middle"></a>
            </td>
    ...
    <?php
	$Qpaging = $this->db->query("SELECT a.`atp_id` FROM `it_atp` a JOIN `it_site` b ON b.`site_id`=a.`site_id` WHERE a.`fl_status`=0".$filter); // AND a.`user_indosat`='".$_SESSION['isat']['uid']."'
	$num_page = ceil($Qpaging->num_rows / $limit);
	if($Qpaging->num_rows > $limit) $this->ifunction->paging($p, base_url().'index.php/main/atp_app/'.$t.'/', $num_page, $Qpaging->num_rows, 'href', '/'.$s.'/'.$sort);
	}else{
