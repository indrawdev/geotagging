...
$Qlist = mysql_query("
	...
	FROM `it_atp` a
	JOIN `it_user` b ON b.`user_id`=a.`user_supervisor` AND b.`fl_active`=1
	WHERE a.`order_supervisor`=3
	GROUP BY a.`user_supervisor`
...
if(mysql_num_rows($Qlist)){
	while($list = mysql_fetch_object($Qlist)){
		...
		$i = 0;
		$lists = '';
		$Qtask = mysql_query("SELECT `title`, `brand` FROM `it_atp` WHERE `user_supervisor`='$list->user_id' AND `order_supervisor`=3") or die("failed, Sorry, could not execute query");
		...
		$body = 'Notified that you have <b>'.mysql_num_rows($Qtask).'</b> pending approval for Acceptance Test Procedure (ATP) task:<br />';
		$body .= '<ul>'.$lists.'</ul>';
		$body .= 'Please login to Indosat Geotagging System (<a href="https://indosat-geotaging.com/index.php/main/atp/page/1/0/vendor-desc" style="color:#0079C0;text-decoration:none">ATP Management</a>).';
		...
		$mail->Subject = 'Reminder Approval - ATP';
...
$Qlist = mysql_query("
	SELECT b.`user_id`, b.`name`, b.`email`
	FROM `it_atp` a
	JOIN `it_user` b ON b.`user_id`=a.`user_manager` AND b.`fl_active`=1
	WHERE a.`order_manager`=3
	GROUP BY a.`user_manager`
") or die("failed, Sorry, could not execute query");
if(mysql_num_rows($Qlist)){
	while($list = mysql_fetch_object($Qlist)){
		...
		$i = 0;
		$lists = '';
		$Qtask = mysql_query("SELECT `title`, `brand` FROM `it_atp` WHERE `user_manager`='$list->user_id' AND `order_manager`=3") or die("failed, Sorry, could not execute query");
		while($task = mysql_fetch_object($Qtask)){
			if($i < 25){
				$lists = '<li>'.$task->title.' ('.$task->brand.')</li>'.$lists;
				$i++;
		...
		$body = 'Notified that you have <b>'.mysql_num_rows($Qtask).'</b> pending approval for Acceptance Test Procedure (ATP) task:<br />';
		$body .= '<ul>'.$lists.'</ul>';
		$body .= 'Please login to Indosat Geotagging System (<a href="https://indosat-geotaging.com/index.php/main/atp/page/1/0/consultant-desc" style="color:#0079C0;text-decoration:none">ATP Management</a>).';
...
/*$Qlist = mysql_query("
	...
	WHERE a.`order_indosat`=3
	GROUP BY a.`user_indosat`
") or die("failed, Sorry, could not execute query");*/
$Qtask = mysql_query("SELECT `title`, `brand` FROM `it_atp` WHERE `order_indosat`=3") or die("failed, Sorry, could not execute query");
if(mysql_num_rows($Qtask)){
		
	$i = 0;
	$lists = '';
	//$Qtask = mysql_query("SELECT `title`, `brand` FROM `it_atp` WHERE `user_indosat`='$list->user_id' AND `order_indosat`=3") or die("failed, Sorry, could not execute query");
	while($task = mysql_fetch_object($Qtask)){
		if($i < 25){
			$lists = '<li>'.$task->title.' ('.$task->brand.')</li>'.$lists;
			$i++;
	...
	while($list = mysql_fetch_object($Qlist)){
		...
		$body = 'Notified that you have <b>'.mysql_num_rows($Qtask).'</b> pending approval for Acceptance Test Procedure (ATP) task:<br />';
		$body .= '<ul>'.$lists.'</ul>';
		$body .= 'Please login to Indosat Geotagging System (<a href="https://indosat-geotaging.com/index.php/main/atp/page/1/0/indosat-desc" style="color:#0079C0;text-decoration:none">ATP Management</a>).';
